# R-Exams-AI: Generación Paramétrica de Exámenes con Inteligencia Artificial
...
En lugar de programar los Procesos de Generación de Datos (DGP) manualmente, se emplean modelos de lenguaje para estructurar la lógica matemática y las variables aleatorias.
...
* El modelo generará un archivo con extensión `.Rmd` (R Markdown) que contendrá el código R para las variables aleatorias y la notación matemática en formato LaTeX.
* Guarde el archivo `.Rmd` resultante dentro de su directorio `Mi_DGP_Quiz`.

### 3. Configuración de Parámetros de Evaluación
...
* **Canvas LMS (`exams2canvas`):** Construye un paquete comprimido (`.zip`) bajo el estándar QTI. Esta configuración utiliza el parámetro `pandoc-mathjax` para garantizar la compatibilidad matemática en el navegador y está listo para ser importado directamente a la plataforma.
